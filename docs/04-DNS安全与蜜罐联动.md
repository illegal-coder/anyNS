...
1. 显式可信 allowlist。
2. 外部 DNSLog/OOB 回调平台拒绝。
3. 通用 denylist、sinkhole 和分析器规则。

该顺序允许受控测试域在明确允许时绕过平台拒绝；否则命中平台域名或其任意
子域时返回 `block`。匹配只使用规范化 DNS 标签边界：大小写和尾点被忽略，
Unicode 域名转换为 IDNA ASCII；`evilinteractsh.com` 不匹配
`interactsh.com`，`token.interactsh.com` 匹配。禁止子串匹配、用户提供正则、
wildcard 扩展和执行型规则。
...
外部 DNSLog 平台拒绝命中时，DNSLog 使用 `matched_rule` =
`external-dnslog-platform`、`action` = `block`、`risk_level` = `high`、
`source_plugin` = `security`。这是 anyNS 内部 DNSLog 审计事件；它不代表
关闭或替代本地 DNSLog 存储。
...
- 异常 qtype 高频查询触发拦截。
- 高熵或普通子域命中外部 DNSLog/OOB 平台域名时被阻断并生成 distinct
  DNSLog 规则；关闭 `reject_dnslog_platforms` 后同一名称不因该规则阻断。
- 平台列表持久化、大小写、尾点、IDNA、重复条目、wildcard/list poisoning、
  边界绕过和 allowlist 优先级均有回归测试。
