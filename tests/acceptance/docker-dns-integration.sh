...
if ! tools 'curl -fsS http://anyns-plugin-runtime:8081/healthz >/dev/null'; then
  ...
  docker compose -p "$PROJECT" -f "$COMPOSE_FILE" logs --no-color anyns-plugin-runtime pdns-recursor bind-latest || true
  exit 1
fi
...
tools 'grep -Eq "status: NXDOMAIN|status: SERVFAIL" /tmp/pdns-missing-hns.txt'
