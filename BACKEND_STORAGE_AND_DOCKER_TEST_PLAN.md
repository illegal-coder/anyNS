...
- `backend-fixtures`: deterministic Python HTTP fixture server for current HNS runtime-json responses, including a private-address rebinding sample, fake Namecoin Core JSON-RPC `.bit` responses, Stacks/Hiro BNS API-style `.btc` zonefile responses, Unstoppable Domains Resolution API-style `.crypto` responses, and a failing honeypot endpoint. It can be extended for ENS, Polkadot PNS, PulseChain PNS, and Wave 2/3 adapters.
