...
| Solana SNS | Implemented | QuickNode/SNS JSON-RPC or fake JSON-RPC fixture; deterministic Docker fixture now covers the current adapter contract | Avoid local Solana node | P2 |
...
| SuiNS | Implemented | Sui JSON-RPC endpoint or fake JSON-RPC fixture; deterministic Docker fixture now covers the current adapter contract | Avoid local Sui node | P2 |
| Freename/FNS | Implemented | Freename Resolution API or fake HTTP fixture; deterministic Docker fixture now covers the current adapter contract | No local chain node | P3 |
| RIF/RNS | Implemented | RSK JSON-RPC or fake JSON-RPC fixture; deterministic Docker fixture now covers the current adapter contract | Avoid local RSK node | P3 |
...
- `backend-fixtures`: deterministic Python HTTP fixture server for current HNS runtime-json responses, including a private-address rebinding sample, fake Namecoin Core JSON-RPC `.bit` responses, ENS JSON-RPC `.eth` responses, PulseChain PNS JSON-RPC `.pls` responses, RIF/RNS JSON-RPC `.rsk` responses, FIO Chain API `.fio` responses, OpenAlias DNS TXT `.openalias` responses, ADA Handle API `.ada` responses, Stacks/Hiro BNS API-style `.btc` zonefile responses, Unstoppable Domains Resolution API-style `.crypto` responses, PNS-Polkadot REST API-style `.dot` responses, SPACE ID Web3 Name API-style `.bnb` responses, Solana SNS JSON-RPC `.sol` responses, TON Center v3 DNS API-style `.ton` responses, Tezos Domains GraphQL `.tez` responses, Aptos Names REST `.apt` responses, SuiNS JSON-RPC `.sui` responses, Freename Resolution API-style `.fns` responses, and a failing honeypot endpoint. It can be extended for more Wave 3 adapters.
...
- After the first routed PowerDNS query, Recursor logs show the Lua hook saw the HNS query and do not contain the Lua dependency fallback for missing `socket.http`, `ltn12`, or `cjson.safe`.
...
- `dig @pdns-recursor alice.sol TYPE262` confirms routed PowerDNS posture for the Solana SNS fixture while runtime HTTP `alice.sol WALLET` returns deterministic `solana-sns-quicknode` wallet records.
...
- `dig @pdns-recursor alice.sui TYPE262` confirms routed PowerDNS posture for the SuiNS fixture while runtime HTTP `alice.sui WALLET` returns deterministic `suins-json-rpc` wallet records.
- `dig @pdns-recursor alice.fns TXT` and runtime HTTP `alice.fns WALLET` return deterministic Freename fixture records through the `freename-resolution-api` adapter contract.
- `dig @pdns-recursor alice.rsk TXT` and runtime HTTP `alice.rsk WALLET` return deterministic RIF/RNS fixture records through the `rif-rns-json-rpc` adapter contract.
...
- Admin, runtime, and log-forwarder audit event reads honor explicit `order=asc` / `order=desc` query modes while preserving the default latest-window chronological response shape.
- Admin, runtime, and log-forwarder audit event reads honor case-insensitive `qname_contains` filters for partial DNSLog investigations while preserving exact `qname` filters for precise lookups.
- Admin, runtime, and log-forwarder audit event reads preserve the legacy JSON array response by default and support opt-in cursor-page envelopes with `page=true` or `cursor=<offset>` for bounded follow-up investigations.
...
5. Done: add `tests/acceptance/docker-dns-integration.sh` that:
   - checks Docker availability,
   - starts the compose stack,
   - runs DNS assertions from `dns-tools`,
   - collects logs on failure,
   - skips cleanly if Docker networking is unavailable.
   - Current scripted assertions cover HNS success, explicit PowerDNS Lua hook dependency fallback detection in Recursor logs, strict HNS `NXDOMAIN`, PowerDNS-routed Namecoin `.bit`, runtime-routed Namecoin subdomain data, PowerDNS/runtime-routed ENS `.eth` fixture records, PowerDNS/runtime-routed Stacks BNS `.btc` fixture records, PowerDNS/runtime-routed Unstoppable `.crypto` fixture records, PowerDNS/runtime-routed PNS-Polkadot `.dot` fixture records, PowerDNS/runtime-routed PulseChain PNS `.pls` fixture records, PowerDNS/runtime-routed RIF/RNS `.rsk` fixture records, PowerDNS/runtime-routed FIO Handle `.fio` fixture records, PowerDNS/runtime-routed OpenAlias `.openalias` fixture records, PowerDNS/runtime-routed ADA Handle `.ada` fixture records, PowerDNS-routed/runtimed SPACE ID `.bnb` fixture records, PowerDNS/runtime-routed Solana SNS `.sol` fixture records, PowerDNS/runtime-routed TON DNS `.ton` fixture records, PowerDNS/runtime-routed Tezos Domains `.tez` fixture records, PowerDNS/runtime-routed Aptos Names `.apt` fixture records, PowerDNS/runtime-routed SuiNS `.sui` fixture records, PowerDNS/runtime-routed Freename/FNS `.fns` fixture records, HNS `WALLET` and `TYPE262`, BIND-forwarded HNS, ICANN pass-through posture, authenticated admin-to-runtime proxy visibility, authenticated proxied admin plugin listing, redacted management-key metadata, admin/runtime policy reload authz plus management audit visibility, proxied admin cache stats/flush authz plus management audit visibility, admin/runtime/log-forwarder audit time-window filtering, admin/runtime/log-forwarder audit-ordering checks, runtime/log-forwarder audit `qname_contains` filtering, runtime/log-forwarder audit cursor pages, admin/runtime/log-forwarder audit-summary aggregates, runtime security denylist/sinkhole/rebinding/reflection-rate-limit policy behavior, authenticated Namecoin/ENS/Stacks/Unstoppable/PNS-Polkadot/PulseChain PNS/RIF-RNS/FIO/OpenAlias/ADA Handle/SPACE ID/Solana SNS/TON DNS/Tezos Domains/Aptos Names/SuiNS/Freename audit reads, runtime honeypot failed-queue metrics, and log-forwarder DNSLog ingestion/authenticated audit/authenticated honeypot-status/metrics/failed-queue behavior through the deterministic failing honeypot fixture.
   - Verified from an empty container stack with fresh local image builds on 2026-06-11.
...
7. Continue Namecoin path in two phases:
   - done: deterministic Namecoin JSON-RPC fixture for current adapter, including PowerDNS-routed DS delegation coverage and no-socket wildcard TLSA adapter coverage,
   - optional `ncdns`/Electrum-NMC or Namecoin Core integration after storage and setup cost is measured.
8. Done: add deterministic d.id/.bit runtime-json fixture coverage using an exact `alice.did.bit` route so the fixture exercises d.id without overriding the broad Namecoin `.bit` route.
