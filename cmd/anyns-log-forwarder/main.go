...
func main() {
	...
	mux.HandleFunc("/api/v1/audit/events", func(w http.ResponseWriter, r *http.Request) {
		...
		if httpapi.AuditEventPageRequested(r) {
			httpapi.WriteJSON(w, http.StatusOK, store.ListFilteredPage(filter, limit, httpapi.AuditEventCursor(r)))
			return
		}
		httpapi.WriteJSON(w, http.StatusOK, store.ListFiltered(filter, limit))
	})
