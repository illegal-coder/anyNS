...
- [x] GitHub Actions `CI` 验证 Go test/vet/build、前端 unit/ESLint/build、源码策略扫描、shell 语法及全部隔离 Compose model（含 SOA/TLD），并上传短期构建产物和 source-policy 摘要；Linux 二进制产物现包含 `anyns-source-policy`。
